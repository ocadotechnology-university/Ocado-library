...
/** Demo corpus for partial-match search suggestions. */
...
const CatalogHomeHeader = ({
  ...
  const suggestions = useMemo(() => {
    ...
    return SEARCH_CORPUS.filter(
      (item) =>
        item.text.toLowerCase().includes(q) || item.hint.toLowerCase().includes(q),
    ).slice(0, 8);
  }, [query]);
              ...
              >
                {suggestions.map((s) => (
                  <li key={s.id} role="option">
                    ...
                    >
                      <span className="font-medium text-[#43485e]">{s.text}</span>
